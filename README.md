...
| **作者共享挂机宝** | 基础配置 | 1元/月 | 专为续火花脚本优化，极低成本，适合只挂脚本的学生党 |

<a id="author-shared-vps"></a>
### 💸 作者共享挂机宝购买

为方便只需挂续火花脚本的用户，作者提供**低价共享挂机宝**，专为抖音续火脚本优化，仅需 **1元/月**，非常适合学生党或只需挂脚本的用户。

**👉 购买与详情**：请访问 [https://dyxh.503555.xyz](https://dyxh.503555.xyz) 了解最新动态与购买方式。

**作者在线时间**：
- **周末**：周日下午 13:00 ~ 17:00
- **法定节假日**：全天在线（节假日最后一天 17:00 后可能离线）
- **上学期间**：工作日不在线，周末按上述时间

> 💡 若未及时回复，请通过网站内提供的交流群或邮箱（syybili@163.com）留言，作者会在上线后第一时间处理。

**免费技术支持**：购买/内测用户均可享受免费脚本配置、问题排查等技术帮助，确保您的挂机体验顺畅无忧。
...
### 2026.04.03 - 用户查找逻辑修复与优化
- 🐛 **修复好友修改昵称导致的bug**：自动检测并更新目标用户列表中的昵称，避免因昵称变化导致查找失败
- 🔍 **优化虚拟列表用户选择**：针对好友列表过多的场景，采用滚动查找策略，确保“选择用户”面板能加载完整好友列表
- 🧹 **删改冗余逻辑**：精简代码结构，移除过时或重复的判断，提升执行效率
- 👥 **贡献者**：感谢 [@YsKiKi](https://github.com/YsKiKi) 提交 PR #9，为本项目带来关键修复与优化

> ⚠️ **已知限制**：若存在两个或两个以上好友昵称相同，脚本会优先匹配列表中靠前的用户，尚需后续版本进一步优化。

### 2026.03.22 - 体验优化与稳定性提升
- 🎯 **自动切换全部标签页**: 解决发送完群消息后无法切换到朋友私信的问题，确保能查找到所有目标用户
- 🗑️ **精简冗余日志**: 移除每5秒输出的查找进度日志，将“未找到输入框”的重复日志降频输出，提升日志可读性
- 🔧 **核心问题修复**: 修复多标签页切换时的状态管理问题，优化查找逻辑
...
### 2026.03.22 - 体验优化与稳定性提升
1. **自动切换全部标签页**: 确保脚本能同时查找到朋友私信和群消息中的用户，解决发送切换问题
2. **精简冗余日志**: 移除高频率无用的日志输出，提升可读性
3. **关键问题修复**: 优化多标签页状态管理
...
**💡 升级建议**: 强烈建议所有用户升级到 2026.03.22 版本！该版本修复了多标签页切换问题，并精简了日志输出，让使用体验更加流畅。
